...
def integrate_result(path_sdf_result, mol_name, ligand_name):
    ...
    for mol in mols:
        ...
        result.append([mol_name, idx, name, score, Chem.MolToSmiles(mol), best, mol])
        idx += 1
        best = False
...
def main(args):
    ...
    #step3 get results
    results_df = pd.DataFrame(results, columns=['ligand_name', 'conformer_idx', 'conformer_name', 'score', 'smiles', 'lowest_conformer', 'mol'])
    results_df.sort_values('score', ascending=True, inplace=True, ignore_index=True)
    ...
    #         f.write(mol)
    # results_df.drop(columns=['mol'], inplace=True)
    # results_df.to_csv(os.path.join(args.output_dir, f"out_vina_docking_{args.filename}_{args.job_id}.csv"), index=False)
